    ...
    const handleCreateSquad = async (e) => {
        ...
        try {
            ...
            const member = await base44.entities.SquadMember.create({
                squad_id: squad.id,
                user_id: user.id,
                player_name: user.data?.player_name || user.data?.full_name || user.full_name,
                role: 'leader',
                last_payout_week: ''
            });
    ...
    const handleJoinSquad = async (squadId) => {
        ...
        try {
            ...
            await base44.entities.SquadMessage.create({
                ...
                user_id: 'system',
                player_name: 'SYSTEM',
                content: `${user.data?.player_name || user.data?.full_name || user.full_name} has joined the squad!`
            });
    ...
    const handleLeaveSquad = async () => {
        ...
        try {
            ...
            await base44.entities.SquadMessage.create({
                ...
                user_id: 'system',
                player_name: 'SYSTEM',
                content: `${user.data?.player_name || user.data?.full_name || user.full_name} has left the squad.`
            });
