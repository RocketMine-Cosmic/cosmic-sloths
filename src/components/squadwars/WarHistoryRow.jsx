...
export default function WarHistoryRow({ war, mySquadId, onClaim, claiming, myWalletLower }) {
    ...
    return (
        <div className="bg-slate-900/60 border border-slate-700 rounded-lg p-3 flex items-center gap-3">
            <div className="text-[10px] font-mono text-slate-500 w-16 shrink-0">{war.week_id}</div>
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
                {!isBye && (
                    <>
                        <span className="text-lg shrink-0 w-6 h-6 inline-flex items-center justify-center overflow-hidden rounded bg-slate-800">
                            {oppIcon?.startsWith('http') ? <img src={oppIcon} className="w-full h-full object-cover" alt="" /> : (oppIcon || '🛡️')}
                        </span>
                        <span className="text-xs text-white font-bold truncate">vs {oppName} <span className="text-slate-500">[{oppTag}]</span></span>
                    </>
                )}
                {isBye && <span className="text-xs text-slate-400 italic">No opponent</span>}
            </div>
