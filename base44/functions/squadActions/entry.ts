...
Deno.serve(async (req) => {
    try {
        ...
        if (action === 'join') {
            ...
            const member = await db.entities.SquadMember.create({ squad_id: squadId, wallet_address: walletAddress, player_name: playerName || 'Pilot', player_title: playerTitle || '', role: 'member', last_payout_week: '', last_daily_payout_date: '' });
            await db.entities.SquadMessage.create({ squad_id: squadId, wallet_address: 'system', player_name: 'SYSTEM', content: `${playerName || 'A pilot'} has joined the squad!` });
            const updatedSquad = await db.entities.Squad.update(squadId, { member_count: (squad.member_count || 0) + 1 });
        ...
        if (action === 'leave') {
            ...
            await db.entities.SquadMessage.create({ squad_id: squadId, wallet_address: 'system', player_name: 'SYSTEM', content: `${playerName || 'A pilot'} has left the squad.` });
            await db.entities.Squad.update(squadId, { member_count: Math.max(0, (squad.member_count || 1) - 1) });
        ...
        if (action === 'kick') {
            ...
            if (leaderRecords.length === 0 || leaderRecords[0].role !== 'leader') return Response.json({ error: 'Only the squad leader can kick members.' }, { status: 403 });
            if (targetMember.wallet_address === walletAddress) return Response.json({ error: 'Cannot kick yourself.' }, { status: 400 });
            await db.entities.SquadMember.delete(targetMemberId);
            await db.entities.Squad.update(squadId, { member_count: Math.max(0, (squad.member_count || 1) - 1) });
            await db.entities.SquadMessage.create({ squad_id: squadId, wallet_address: 'system', player_name: 'SYSTEM', content: `${targetMember.player_name} was removed from the squad.` });
            return Response.json({ success: true });
        }
        ...
        if (action === 'sendMessage') {
            ...
            const message = await db.entities.SquadMessage.create({ squad_id: squadId, wallet_address: walletAddress, player_name: playerName || 'Pilot', player_title: playerTitle || '', content: content.substring(0, 200) });
        ...
        if (action === 'transferLeadership') {
            ...
            if (leaderRecords.length === 0 || leaderRecords[0].role !== 'leader') return Response.json({ error: 'Only the squad leader can transfer leadership.' }, { status: 403 });
            await Promise.all([
            ...
            await db.entities.SquadMessage.create({ squad_id: squadId, wallet_address: 'system', player_name: 'SYSTEM', content: `${targetMember.player_name} is now the squad leader!` });
        ...
        if (action === 'saveSettings') {
            ...
            if (leaderRecords.length === 0 || leaderRecords[0].role !== 'leader') return Response.json({ error: 'Only the squad leader can change settings.' }, { status: 403 });
            const updated = await db.entities.Squad.update(squadId, { name: name?.trim(), tag: tag?.trim().toUpperCase().substring(0, 4), description: description?.trim() || '', icon: icon || '🛡️' });
